...
static ssize_t ssam_kip_sw_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static struct device_attribute ssam_kip_sw_state =
	__ATTR(state, 0444, ssam_kip_sw_state_show, NULL);

static struct attribute *ssam_kip_sw_attrs[] = {
	&ssam_kip_sw_state.attr,
	NULL,
};
